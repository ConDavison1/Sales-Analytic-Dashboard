...
@executives_bp.route('/revenue-performance', methods=['GET'])
def get_revenue_performance():
    """
    Get revenue performance indicators for all account executives
    
    Returns the total revenue generated by each account executive for the specified year,
    along with win count and revenue from signings.
    ...
    try:
        ...
        ae_data = get_account_executives_revenue_data(user.user_id, year)
        ...
        # Return formatted response
        return jsonify({
            "revenue_performance": ae_data,
            "year": year,
            "total_revenue": round(total_revenue, 2)
        }), 200
        
    except Exception as e:
        print(f"Error in get_revenue_performance: {str(e)}")
        return jsonify({"error": f"Failed to retrieve revenue performance data: {str(e)}"}), 500


def get_account_executives_revenue_data(director_id, year):
    """
    Get revenue data for all account executives under a director
    ...
    Returns:
        List of dictionaries with account executive revenue performance data
    """
    # Get all account executives managed by this director
