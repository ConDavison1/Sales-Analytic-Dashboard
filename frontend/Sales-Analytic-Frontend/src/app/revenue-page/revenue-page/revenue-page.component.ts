...
export class RevenuePageComponent implements OnInit {
  ...
  constructor(private dashboardService: DashboardService, private revenueService: RevenueService) {}
  ...
  updateBarChartData(): void {
    ...
    if (this.revenueChartData.length > 0) {
      this.barChart.series[0].data = this.revenueChartData.map(item => item.count);
      this.barChart.xaxis.categories = this.revenueChartData.map(item => item.category);
    }
  }
